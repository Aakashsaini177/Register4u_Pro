// Fix EX1001 photo URL in file manager
const mongoose = require('mongoose');
require('dotenv').config();

async function fixEX1001Photo() {
  try {
    console.log('🔧 Fixing EX1001 photo URL in file manager...');
    ...
    // Find EX1001 visitor
    const visitor = await Visitor.findOne({ visitorId: 'EX1001' });
    
    if (!visitor) {
      console.log('❌ EX1001 visitor not found');
      return;
    }
    
    console.log('👤 Found visitor EX1001:');
    console.log(`   Name: ${visitor.name}`);
    console.log(`   Photo URL: ${visitor.photo}`);
    
    // Find EX1001 file in file manager
    const photoFile = await FileNode.findOne({ 
      name: 'EX1001.jpg'
    });
    
    if (!photoFile) {
      console.log('❌ EX1001.jpg file not found in file manager');
      return;
    }
