// Fix OP1001 photo URL in file manager
const mongoose = require('mongoose');
require('dotenv').config();

async function fixOP1001Photo() {
  try {
    console.log('🔧 Fixing OP1001 photo URL in file manager...');
    ...
    // Find OP1001 visitor
    const visitor = await Visitor.findOne({ visitorId: 'OP1001' });
    ...
    console.log('👤 Found visitor OP1001:');
    console.log(`   Name: ${visitor.name}`);
    console.log(`   Photo URL: ${visitor.photo}`);
    
    // Find OP1001 file in file manager
    const photoFile = await FileNode.findOne({ 
      name: 'OP1001.jpg'
    });
    
    if (!photoFile) {
      console.log('❌ OP1001.jpg file not found in file manager');
      return;
    }
