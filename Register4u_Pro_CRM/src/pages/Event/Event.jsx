    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage your events</p>
        </div>
        <Link to="/event/add">
      ...
      <Card>
        <CardContent className="p-6">
          <div className="relative">
            ...
            <Input type="text" placeholder="Search events..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </CardContent>
      </Card>
              ...
              <TableBody>
                {events.map((event) => (
                  <TableRow key={event.id}>
                    ...
                    <TableCell>{event.eventName || event.name || 'N/A'}</TableCell>
                    <TableCell>{formatDate(event.StartTime || event.date)}</TableCell>
                    <TableCell>{event.location || 'N/A'}</TableCell>
                    <TableCell>
                      <Badge variant="success">{event.status || 'Active'}</Badge>
                    </TableCell>
                    <TableCell>
