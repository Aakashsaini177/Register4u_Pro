...
const ImageCropper = ({ imageSrc, onCrop, onCancel, aspect = 3/4, freeAspect = false, title = "Adjust Photo" }) => {
  const imageRef = useRef(null);
  const containerRef = useRef(null);
  
  const [imageLoaded, setImageLoaded] = useState(false);
  const [zoom, setZoom] = useState(1);
  const [cropArea, setCropArea] = useState({ x: 50, y: 50, width: 200, height: 200 * (freeAspect ? 1 : (1/aspect)) });
  const [isDragging, setIsDragging] = useState(false);
  const [isResizing, setIsResizing] = useState(false);
  const [resizeDirection, setResizeDirection] = useState(null);

  const [dragStart, setDragStart] = useState({ x: 0, y: 0 });
  const [resizeStart, setResizeStart] = useState({ x: 0, y: 0, cropArea: null });
  ...
  const handleMouseMove = (e) => {
    ...
    if (isDragging) {
      ...
      newCrop.y = Math.max(0, Math.min(rect.height - newCrop.height, newCrop.y));
  ...
  }, [isDragging, isResizing, cropArea, dragStart, resizeStart, resizeDirection, aspect, freeAspect]);
      ...
      <div className="w-full max-w-lg bg-gray-900 rounded-lg overflow-hidden flex flex-col max-h-[98vh] min-h-[500px]">
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 0 0 1px rgba(255,255,255,0.3), 0 0 10px rgba(59, 130, 246, 0.3)'
                }}
              ...
              >
                ...
                {/* Center Indicator */}
                <div className="absolute top-1/2 left-1/2 w-1 h-1 bg-white rounded-full pointer-events-none" style={{ transform: 'translate(-50%, -50%)' }} />
              </div>
            </>
          )}
        ...
        <div className="p-4 bg-gray-900 space-y-4 z-10 flex-shrink-0">
          ...
          <div className="text-xs text-gray-400 text-center bg-gray-800 p-2 rounded-lg">
            ...
            <div>🖱️ <strong>Drag</strong> blue box to move crop area</div>
            <div>📐 <strong>Drag corners/edges</strong> to resize</div>
            <div>🔍 <strong>Mouse wheel</strong> to zoom image</div>
            <div>🎯 <strong>Double click</strong> to center crop area</div>
            {!freeAspect && (
            ...
            <div className="text-gray-500 mt-2">
              Zoom: {zoom.toFixed(1)}x • Crop: {Math.round(cropArea.width)}×{Math.round(cropArea.height)}
              {!freeAspect && ` • Ratio: ${(cropArea.width/cropArea.height).toFixed(2)}:1`}
            </div>
            ...
            {title.includes('Free Cropping') && (
              <div className="text-green-400 mt-1 text-xs">
                📐 <strong>Output:</strong> High Quality (min 800px on longer side)
              </div>
            )}
          </div>
